...
fn download_tickers(pool: &ThreadPool, polygon: &mut PolygonClient, date: NaiveDate, progress: Arc<Mutex<Progress>>, dir: &str) -> Vec<String> {
	let now = Instant::now();
...
fn download_trades(pool: &ThreadPool, polygon: &mut PolygonClient, date: NaiveDate, progress: Arc<Mutex<Progress>>, dir: &str, tickers: Vec<String>) {
	let now = Instant::now();
	let trades_path = format!("{}/{}.csv.zst", dir, date);
	if Path::new(&trades_path).exists() {
		debug!("{} exists, skipping", trades_path);
	} else {
		info!("Downloading trades for {}", date);
...
fn main() {
  ...
  for date in market_days.into_iter().rev() {
		let tickers = download_tickers(&tickers_pool, &mut polygon, date, progress.clone(), &tickers_dir);
		...
		tickers_pool.execute(move|| {
			download_trades(&trades_pool, &mut polygon, date, progress, &trades_dir, tickers);
		});
  }
