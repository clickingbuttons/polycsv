...
pub fn is_market_open(date: &NaiveDate) -> bool {
  ...
  // Juneteenth becomes holiday
  if year >= 2022 {
    let mut juneteenth = NaiveDate::from_ymd(year, 6, 19);
    if juneteenth.weekday() == Sat {
      juneteenth -= Duration::days(1);
    } else if juneteenth.weekday() == Sun {
      juneteenth += Duration::days(1);
    }
    if date == &juneteenth {
      return false;
...
mod tests {
  ...
  #[test]
  fn juneteenth() {
    assert_eq!(is_market_open(&NaiveDate::from_ymd(2022, 6, 20)), false);
  }
}
